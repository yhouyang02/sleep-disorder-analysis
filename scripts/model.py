...
@click.option('--output-prefix', default='results/model_analysis', help='Prefix for output files (e.g. results/this_analysis)')
def run_model(input_file, output_prefix):
    ...
    save_df_as_png(cv_results_df.round(4), f"{output_prefix}_cv_results.png", "Cross-Validation Results (Ridge)")
    ...
    save_df_as_png(results_df.round(4), f"{output_prefix}_test_metrics.png", "Test Set Evaluation")
    ...
    ax1.set_title('Actual vs Predicted Stress Levels', fontsize=14, fontweight='bold', loc='left')
    ax1.legend()
