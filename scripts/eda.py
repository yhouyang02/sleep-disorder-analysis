...
@click.command()
@click.option('--input-file', default='data/processed/clean_sleep_data.csv', help='Path to the cleaned data file')
...
def perform_eda(input_file, output_dir):
    ...
    ax1.text(0.01, 1.02, '(a)', color='red', fontsize=20, fontweight='bold', transform=ax1.transAxes)
    ...
    ax2.text(0.01, 1.02, '(b)', color='red', fontsize=20, fontweight='bold', transform=ax2.transAxes)
    ...
    ax3.text(0.01, 1.02, '(c)', color='red', fontsize=20, fontweight='bold', transform=ax3.transAxes)
    ...
    if len(train_df) > 1 and train_df['stress_level'].nunique() > 1:
        ...
        x_range = np.linspace(train_df['stress_level'].min(), train_df['stress_level'].max(), 200)
        y_kde = kde(x_range)
    ...
    ax4.text(0.01, 1.02, '(d)', color='red', fontsize=20, fontweight='bold', transform=ax4.transAxes)
